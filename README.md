...
天気、雨雲レーダー、警報・注意報、地震、津波、台風、花粉、生活指数をまとめて確認するための個人向け天気ダッシュボードです。

花粉だけは提供元 (ウェザーニュース) の条件により、インターネットに公開したURLでは表示しません。手元 (`localhost` やファイルを直接開いた場合) と Android アプリ版でのみ表示されます。詳しくは[花粉データの扱い](#花粉データの扱い)を参照してください。
...
表示が古い、または更新されない場合は、アプリ内の「すべて更新」を押してください (警報・地震・津波・台風・雨雲・天気・各社予報をまとめて取り直します。花粉は表示している場合のみ)。それでも変わらない場合は、ブラウザで再読み込みするか、ホーム画面から開いている場合はいったん閉じて開き直してください。
...
- Weathernews WxTech Open Data: 花粉観測データ (公開URLでは非表示。下記参照)
- Leaflet: 地図表示ライブラリ
...
花粉データ (ウェザーニュース ポールンロボ観測) の提供条件に、次の記載があります。

> 営利・商用目的でのご利用や、インターネットを通じた成果物の公開をご検討の場合は、お問い合わせください
> — [花粉飛散数データの無料ダウンロード](https://wxtech.weathernews.com/products/data/services/opendata-pollen/)

このリポジトリは GitHub Pages で公開しているため、公開URLで開いた場合は花粉タブを表示せず、ウェザーニュースへの通信も行いません。表示するのは次の場合だけです。

- `localhost` / `127.0.0.1` で開いたとき
- HTMLファイルを直接 (`file://`) 開いたとき
- Android アプリ版 (WebViewのネイティブ連携がある状態)

判定はソース内の `POLLEN_ENABLED` で行っています。公開せずに自分だけで使う場合は、リポジトリを手元に置いて `localhost` で開けば従来どおり花粉も使えます。

他のデータ (気象庁・国土地理院・Open-Meteo) に公開の制限はありません。
